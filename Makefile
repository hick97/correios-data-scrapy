...
env-ip:
	...
	echo $$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' correios-data-scrapy_mongodb_1)

env-stop:
	-docker-compose down -v

do-test:
	MONGO_URL=$$(docker inspect -f '{{.NetworkSettings.Networks.goteca_default.IPAddress}}' correios-data-scrapy_mongodb_1) \
